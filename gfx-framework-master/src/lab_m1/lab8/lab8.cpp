...
void Lab8::Init()
{
    ...
    // Light & material properties
    {
        ...
        point_light_pos[1] = glm::vec3(1, 1, 1);
...
void Lab8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    GLint type = glGetUniformLocation(shader->program, "is_spot");
    glUniform1i(type, isSpot);
...
void Lab8::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        if (window->KeyHold(GLFW_KEY_UP)) point_light_pos[1] -= forward * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_LEFT)) point_light_pos[1] -= right * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_DOWN)) point_light_pos[1] += forward * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_RIGHT)) point_light_pos[1] += right * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_P)) point_light_pos[1] += up * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_L)) point_light_pos[1] -= up * deltaTime * speed;
    }
}
