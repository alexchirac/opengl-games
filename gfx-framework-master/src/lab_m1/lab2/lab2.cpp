...
void Lab2::Init()
{
    ...
    }
    {
        ...
        mesh->LoadMesh(PATH_JOIN(window->props.selfDir, RESOURCE_PATH::MODELS, "animals"), "MASINA.obj");
        meshes[mesh->GetMeshID()] = mesh;
    }
    ...
    // Create a mesh box using custom data
    {
        vector<VertexFormat> vertices
        {
            ...
            VertexFormat(glm::vec3(0, 0, 0), glm::vec3(0, 0, 1)), //1   //0
            VertexFormat(glm::vec3(0, 0, 0), glm::vec3(0, 0.5f, 0.5f)), //2

            // TODO(student): Complete the vertices data for the cube mesh
            VertexFormat(glm::vec3(0, 0, 1), glm::vec3(1, 0, 0)), //3
            VertexFormat(glm::vec3(0, 0, 1), glm::vec3(0, 1, 0)), //4  //1
            VertexFormat(glm::vec3(0, 0, 1), glm::vec3(0, 0, 1)), //5


            VertexFormat(glm::vec3(1, 0, 0), glm::vec3(0, 1, 0)), //6
            VertexFormat(glm::vec3(1, 0, 0), glm::vec3(0, 0, 1)), //7  //2
            VertexFormat(glm::vec3(1, 0, 0), glm::vec3(0, 0.5f, 0.5f)), //8

            VertexFormat(glm::vec3(0.5f, 1, 0.5f), glm::vec3(1, 0, 0)), //9
            VertexFormat(glm::vec3(0.5f, 1, 0.5f), glm::vec3(0, 1, 0)), //10 //3
            VertexFormat(glm::vec3(0.5f, 1, 0.5f), glm::vec3(0, 0.5f, 0.5f)), //11
        ...
        vector<unsigned int> indices =
        {
            9, 0, 3,    // indices for first triangle
            10, 4, 6,    // indices for second triangle
            // TODO(student): Complete indices data for the cube mesh
            1, 7, 5,
            11, 8, 2,
        ...
        // Actually create the mesh from the data
        CreateMesh("tethr", vertices, indices);
    }
    ...
    // opposing vertex orientations.
    {
        ...
        {
            0, 1, 2,    // indices for first triangle
            3, 1, 2,    // indices for second triangle
...

    {
        ...
        int nr = 50;
        for (int i = 0; i < nr; i++) {
            vertices.push_back(VertexFormat(glm::vec3(0, r* sin(i * 2 * AI_MATH_PI / nr), r* cos(i * 2 * AI_MATH_PI / nr))));
        }
...
void Lab2::Update(float deltaTimeSeconds)
{
    ...
    RenderMesh(meshes["teapot"], shaders["VertexNormal"], glm::vec3(0, 0, 0), glm::vec3(0.5f));
    ...
    //RenderMesh(meshes["cube_B"], shaders["VertexColor"], glm::vec3(4, 4, 4));

    // TODO(student): Draw the tetrahedron
    //RenderMesh(meshes["tethr"], shaders["VertexColor"], glm::vec3(3, 0, 0));

    // TODO(student): Draw the square
    //RenderMesh(meshes["square"], shaders["VertexColor"], glm::vec3(0, 0, 2));

    meshes["disc"]->SetDrawMode(GL_TRIANGLE_FAN);
    //RenderMesh(meshes["disc"], shaders["VertexColor"], glm::vec3(0, 0, 5));
