...
void Lab7::Update(float deltaTimeSeconds)
{
    ...
    // Render ground
    {
        ...
        RenderSimpleMesh(meshes["plane"], shaders["LabShader"], modelMatrix, glm::vec3(0.1, 0.7, 0.2));
    }
...
void Lab7::RenderSimpleMesh(Mesh* mesh, Shader* shader, const glm::mat4& modelMatrix, const glm::vec3& color)
{
    ...
    auto loc_light_position = glGetUniformLocation(shader->program, "light_position");
    glUniform3fv(loc_light_position, 1, glm::value_ptr(lightPosition));

    glm::vec3 eyePosition = GetSceneCamera()->m_transform->GetWorldPosition();
    auto loc_eye_position = glGetUniformLocation(shader->program, "eye_position");
    ...
    auto loc_material_kd = glGetUniformLocation(shader->program, "material_kd");
    glUniform1f(loc_material_kd, materialKd);

    auto loc_material_ks = glGetUniformLocation(shader->program, "material_ks");
    glUniform1f(loc_material_ks, materialKs);

    auto loc_material_shininess = glGetUniformLocation(shader->program, "material_shininess");
    glUniform1i(loc_material_shininess, materialShininess);

    auto loc_object_color = glGetUniformLocation(shader->program, "object_color");
