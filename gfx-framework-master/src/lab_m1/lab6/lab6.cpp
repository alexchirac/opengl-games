...
void Lab6::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix)
{
    ...
    glUniform2f(location, 0.0f, 1.0f);

    glm::vec3 a(1.0f);
    glUniform3f(location, a.x, a.y, a.z);
    glUniform3fv(location, 1, glm::value_ptr(a));*/

    double time = Engine::GetElapsedTime();
    // TODO(student): Set shader uniform "Model" to modelMatrix
    /*glm::mat4 newMat = modelMatrix;
    newMat[0][1] += cos(time);
    newMat[1][2] += sin(time);
    newMat[2][2] += cos(5 * time);*/
    ...
    glUniform3f(location, 0, cos(time), 0);
