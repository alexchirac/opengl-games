...
void Lab9::Update(float deltaTimeSeconds)
{
    ...
    {
        glm::vec3 pos = glm::vec3(0, 0.5, 0);
        glm::vec3 cameraPos = GetSceneCamera()->m_transform->GetLocalPosition();

        glm::vec3 dir = glm::normalize(glm::vec3(cameraPos.x, 0, cameraPos.z));
...
void Lab9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2)
{
    ...
    if (texture2)
    {
        ...
        glUniform1i(glGetUniformLocation(shader->program, "mix_col"), 1);
    }
    else {
        glUniform1i(glGetUniformLocation(shader->program, "mix_col"), 0);
    }
...
Texture2D* Lab9::CreateRandomTexture(unsigned int width, unsigned int height)
{
    ...
    for (int i = 0; i < size; i++) {
        data[i] = (unsigned char)rand() % UCHAR_MAX;
    }
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT); // modul de wrapping pe verticala
